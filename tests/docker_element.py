...
@pytest.mark.docker
@pytest.mark.datafiles(DATA_DIR)
def test_single_build_dep_docker_image(
    cli, docker_client, datafiles, tmp_path
):
    test_element = "hello-world-image.bst"
    project = str(datafiles)
    checkout_dir = os.path.join(str(tmp_path), "checkout")
    hello_exec = os.path.join(
        project, "files", "hello-world-image-fs", "hello"
    )
    ...
    with open(
        os.path.join(project, "files", "hello-world_output.txt")
    ) as expected_output:
...
def test_nested_overwrite_docker_image(
    ...
    extract_path = untar(os.path.join(checkout_dir, "image.tar"))
    assert (
        _get_number_of_file_duplications(_get_layer_files(extract_path)) == 2
    )
    ...
    with tarfile.open(exported_tar) as tar_handle:
        ...
        safe_extract(tar_handle, path=container_fs_export_dir)

    try:
        with open(
            os.path.join(container_fs_export_dir, "layer1", "hello.txt")
        ) as produced_file:
            with open(
                os.path.join(project, "files", "layers", "layer2", "hello.txt")
            ) as actual_file:
...
def _get_layer_files(extract_path):
    ...
    for layer in os.listdir(extract_path):
        if os.path.isdir(os.path.join(extract_path, layer)):
            with tarfile.open(
                os.path.join(extract_path, layer, "layer.tar")
            ) as tar_handle:
...
def _check_meta_data(docker_client, tag):
    ...
    date_created = datetime.strptime(
        image_attrs["Created"], "%Y-%m-%dT%H:%M:%SZ"
    )
